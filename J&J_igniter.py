...
# nominal parameters
Preg_ox          = 970*psi          # regulated GOX outlet pressure [Pa]
Preg_N2          = 970*psi          # regulated N2 outlet pressure [Pa]
...
d_oxtube        = 0.25*inches       # ox tank -> manifold tube diameter [m]
L_oxtube        = 0.6               # ox tank -> manifold tube length [m]
d_fueltube      = 0.25*inches       # fuel tank -> manifold tube diameter [m]
L_fueltube      = 0.6               # fuel tank -> manifold tube length [m]

roughness       = 0.005             # epsilon/diameter, dimensionless

# Valves
Cv_ox_check     = 0.1               # oxidizer check valve flow coefficient, dimensionless
Pcrack_ox_check = 0.33*psi          # oxidizer check valve opening pressure [Pa]
...
Cv_fuel_check   = 0.1               # fuel igniter check valve flow coefficient, dimensionless
Pcrack_fuel_check = 0.33*psi        # fuel igniter check valve opening pressure [Pa]

Cv_pres_valve   = 50                # nitrogen ball valve flow coefficient, dimensionless
Cv_ox_valve     = 0.022             # oxidizer solenoid valve flow coefficient, dimensionless 
Cv_fuel_valve   = 0.07              # fuel solenoid valve flow coefficient, dimensionless

# Injector

cd_oxInjector   = 0.61                                                  # orifice discharge coefficient
diameter_oxInjectorHoles = 600e-6 #number xx drill                       # ox orifice diameter [m]
#length_oxHole   = 0.005                                                # ox orifice length [m]
numOxInjectorHoles = 1                                                  # number of ox orifices in the injector
area_oxInjector = numOxInjectorHoles*pi*diameter_oxInjectorHoles**2/4   # total ox flow area [m2]

cd_fuelInjector = 0.61                                                  # orifice discharge coefficient
diameter_fuelInjectorHoles = 188e-6 #number xx drill              # fuel orifice diameter [m]
numFuelHoles    = 1                                                     # number of fuel orifices in the injector
...
contrac_ratio   = (Dchamber/d_nozzleThroat)**2                          # chamber contraction ratio
...
fuel_check      = rc.IncompressibleFlowCheckValve( Cv_fuel_check, Pcrack_fuel_check, kerosene)
...
P3fuel          = [0]                               # fuel chekck valve inlet pressure [Pa]
...
ox_sole_CF      = [0]                               # how close ox solenoid is to choking (CF>=1 means choked)
ox_inj_CF       = [0]                               # how close ox injector is to choking (CF>=1 means choked)
...
#while True:
    ...
    if i==0:    # First guesses. Based on choked flow at ox injector (mdot_ox_guess multiplied by xx to adjust for better convergence)
        
        mdot_ox_guess   = ox_orifice.getMdot(Preg_ox, Pfire, Tox)*1.0
        ...
        mdot_fuel_guess = fuel_orifice.getMdot(Preg_N2, Pfire, rooFuel)*1.1     #assuming injector limits mdot_fuel
        ...
        P4fuel_guess    = P3fuel_guess - fuel_check.getPressureDrop(mdot_fuel_guess)
        ...
        print("P2fuel_guess is", P2fuel_guess/psi, "psi")
    ...
    def oxfunks(U):       # defines the system of equations and unknowns U to be solved
        ...
        out             = [ (P2ox - P3 - ox_check.getPressureDrop(mdot, P2ox, GOX.roo_std, roo_ox, Tox)) /(P2ox-P3) ]
        #out             = [ (mdot - ox_check.getMdot(P2ox, P3, GOX.roo_std, roo_ox, Tox)) /mdot ]
        out.append( (P3 - P4 - oxSole.getPressureDrop( mdot, P3, roo_ox))                   /(P3-P4) )
        out.append( (P4 - P5 - ox_tube.getPressureDrop(mdot, mu_ox, roo_ox))                /(P4-P5) )
        out.append( (mdot - ox_orifice.getMdot(P5, Pchamb, Tox))                            /mdot )
    ...
    def fuelfunks(U):       # defines the system of equations and unknowns U to be solved
        ...
        out             = [ (mdot - fuelSole.getMdot(P1fuel, P2, rooFuel, kerosene.P_crit, kerosene.P_vapor)) /(mdot)  ]
        out.append( (P2 - P3 - fuel_tube.getPressureDrop(mdot, mu_fuel, rooFuel))                   /(1) )
        out.append( (P3 - P4 - fuel_check.getPressureDrop(mdot))                                    /(1) ) 
        #out.append( (P4 - Pchamb - fuel_orifice.getPressureDrop(mdot, rooFuel))                    /(P4-Pchamb) )
        out.append( (mdot - fuel_orifice.getMdot(P4, Pchamb, rooFuel))                              / (mdot) )
    ...
    fueltank.update(TfuelPres[i], 0, mdot_fuel_new, Preg_N2, timestep) 
    ...
    fueltank.update(TfuelPres[i], mdot_pres_new, 0, Preg_N2, timestep)
    
    # ...and fuel pressurant tank 
    fuelprestank.update(mdot_pres_new, timestep)
    
    # Check if OFratio is within limits. If not, stop simulation (no CEA data beyond OF-ratio 0.5-3.0)
    if (mdot_ox_new/mdot_fuel_new) < 0.5 or (mdot_ox_new/mdot_fuel_new) > 5.0:
        print("OF ratio out of range (OF =", '%.3f'%(mdot_ox_new/mdot_fuel_new), "), terminate")
    ...
    ox_sole_CF.append(oxSole.getCF(ox_solution[0], ox_solution[1]) )
    ox_inj_CF.append(ox_orifice.getCF(ox_solution[2], Pchamber[-1]) )
...
print("SS mdot_ox is", '%.3f'%(mdot_ox[-1]*1000), "g/s")
print("SS mdot_fuel is", '%.3f'%(mdot_fuel[-1]*1000), "g/s")
...
# See what compressible check valves are doing
dP_ox_check     = (Preg_ox - P3ox[-1])
dP_N2_check     = (Preg_N2 - P3pres[-1])
dP_fuel_check   = (P3fuel[-1] - P4fuel[-1])

# See what orifices are doing
print("Ox injector is choked:", ox_orifice.isChoked(P5ox[-1], Pchamber[-1]))
print("Ox solenoid is choked:", oxSole.isChoked(P3ox[-1], P4ox[-1]))
print("")
...
if dP_fuel_check < fuel_check.Pcrack:    
    print("Warning: Pressure drop over fuel check valve(",'%.1f'%(dP_fuel_check/psi),"psi) is less than its cracking pressure (",fuel_check.Pcrack/psi,"psi) and will remain shut")  
...
plt.figure(2)
plt.plot(time,array(P3fuel)/psi, label='Pcheck_in')
plt.figure(2)
...
plt.plot( [8, 9], [y7, y8], linewidth=2, label="Check valve")
...
plt.figure(15)
plt.plot(time, ox_sole_CF, label = "Ox solenoid")
plt.plot(time, ox_inj_CF, label = "Ox injector")
plt.title('choke checks (CF>=1 means choked)')
plt.xlabel('Time [s]')
plt.ylabel('Choke factor CF')
plt.legend( loc ='upper right')
plt.show()